...
{
    int found_player = 0;
    for (int y = 0; y < data->cube->map->height; y++)
    {
        for (int x = 0; x < data->cube->map->width; x++)
        {
            char c = data->cube->map->map[y][x];
            if (c == 'N' || c == 'S' || c == 'E' || c == 'W')
            {
                if (found_player)
                {
                ...
                data->pdy = -sin(degToRad(data->pa));
                found_player = 1;
                data->cube->map->map[y][x] = '0'; // Replace player position with empty space
				data->planeX = -data->pdy * 0.66;
    			data->planeY = data->pdx * 0.66;
            }
        }
